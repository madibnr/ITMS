<x-app-layout>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
        <div>
            <h1 style="font-size: 24px; font-weight: 700; color: var(--text-primary);">Edit Documentation</h1>
            <p style="font-size: 14px; color: var(--text-muted); margin-top: 4px;">
                ...
                / <a href="{{ route('docs.show', $documentation) }}" style="color: var(--text-muted); text-decoration: none;">{{ Str::limit($documentation->title, 40) }}</a>
                / Edit
            </p>
        </div>
    </div>

    <form method="POST" action="{{ route('docs.update', $documentation) }}" enctype="multipart/form-data">
        @csrf @method('PUT')
            ...
            <div style="display: flex; flex-direction: column; gap: 16px;">
                    ...
                    <div class="panel-body" style="padding: 24px;">
                        ...
                        <div style="margin-bottom: 16px;">
                            ...
                            <select name="category_id" id="category_id" required onchange="loadMetaFields(this)" style="width: 100%; padding: 10px 14px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 14px; outline: none;">
                                ...
                                <option value="{{ $cat->id }}" data-slug="{{ $cat->slug }}" {{ (old('category_id', $documentation->category_id) == $cat->id) ? 'selected' : '' }}>{{ $cat->icon }} {{ $cat->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div style="margin-bottom: 16px;">
                            ...
                            <input type="text" name="title" value="{{ old('title', $documentation->title) }}" required style="width: 100%; padding: 10px 14px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 14px; outline: none;">
                        ...
                        <div>
                            ...
                            <input type="text" name="description" value="{{ old('description', $documentation->description) }}" maxlength="1000" style="width: 100%; padding: 10px 14px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 14px; outline: none;">
                        </div>
                    </div>
                </div>
                ...
                <div class="panel">
                    <div class="panel-body" style="padding: 24px;">
                        ...
                        <textarea name="content" id="content-editor" rows="16" style="width: 100%; padding: 10px 14px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 14px; outline: none; resize: vertical;">{{ old('content', $documentation->content) }}</textarea>
                    ...
                    <div class="panel-body" style="padding: 24px;">
                        ...
                        <div style="margin-bottom: 16px;">
                            ...
                            <select name="status" required style="width: 100%; padding: 10px 14px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 14px; outline: none;">
                                <option value="draft" {{ old('status', $documentation->status) === 'draft' ? 'selected' : '' }}>Draft</option>
                                <option value="published" {{ old('status', $documentation->status) === 'published' ? 'selected' : '' }}>Published</option>
                            </select>
                        </div>
                        <div style="margin-bottom: 16px;">
                            <label style="display: block; font-size: 13px; font-weight: 600; color: var(--text-secondary); margin-bottom: 6px;">Tags</label>
                            @php $selectedTags = old('tags', $documentation->tags->pluck('id')->toArray()); @endphp
                        ...
                        <div style="margin-bottom: 16px;">
                            <label style="display: block; font-size: 13px; font-weight: 600; color: var(--text-secondary); margin-bottom: 6px;">Replace Attachment</label>
                            @if($documentation->attachment)
                            <div style="font-size: 12px; color: var(--text-muted); margin-bottom: 6px;">
                                Current: {{ $documentation->attachment_original_name ?? basename($documentation->attachment) }}
                            </div>
                            @endif
                            <input type="file" name="attachment" accept=".pdf,.docx,.xlsx,.png,.jpg,.jpeg" style="width: 100%; padding: 8px 0; font-size: 13px;">
                            <div style="font-size: 11px; color: var(--text-muted); margin-top: 4px;">Leave blank to keep existing file.</div>
                            @error('attachment') <span style="color: #ef4444; font-size: 12px;">{{ $message }}</span> @enderror
                        </div>
                        <div style="display: flex; flex-direction: column; gap: 8px;">
                            <button type="submit" style="background: linear-gradient(135deg, var(--primary), var(--primary-dark)); color: white; padding: 10px 20px; border-radius: 8px; font-size: 14px; font-weight: 600; border: none; cursor: pointer; width: 100%;">Update Document</button>
                            <a href="{{ route('docs.show', $documentation) }}" style="display: block; text-align: center; padding: 10px 20px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 14px; text-decoration: none; color: var(--text-secondary);">Cancel</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        const existingMeta = @json($documentation->meta->pluck('value', 'key'));
