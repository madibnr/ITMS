<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @endif
                    @if(auth()->user()->hasAnyRole(['admin', 'it-staff', 'manager']))
                    ...
                    <x-nav-link :href="route('reports.tickets')" :active="request()->routeIs('reports.*')">
                        Reports
                    </x-nav-link>
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @endif
            @if(auth()->user()->hasAnyRole(['admin', 'it-staff', 'manager']))
            <x-responsive-nav-link :href="route('incidents.index')" :active="request()->routeIs('incidents.*')">Incidents</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('change-requests.index')" :active="request()->routeIs('change-requests.*')">Changes</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('maintenance.index')" :active="request()->routeIs('maintenance.*')">Maintenance</x-responsive-nav-link>
            @endif
        </div>
